...
class GameRenderer {
    ...
    if (world && world.WorldMap && world.WorldMap.Territories) {
      Object.values(world.WorldMap.Territories).forEach((territory: Territory) => {
        ...
        territory.Zones?.forEach((zoneId: string) => {
          ...
          if (zone && zone.X !== undefined && zone.Y !== undefined && zone.Width !== undefined && zone.Height !== undefined) {
  ...
  isPointInZone(x: number, y: number, zone: Zone): boolean {
    return zone.X !== undefined && zone.Y !== undefined && zone.Width !== undefined && zone.Height !== undefined && x >= zone.X && x <= zone.X + zone.Width && y >= zone.Y && y <= zone.Y + zone.Height;
  }
}
